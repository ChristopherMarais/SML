...
# plot k-fold
# calculate max and min limits of data
kfold_max_vec = apply(KFOLD_df[, 2:6], 1, max)
kfold_min_vec = apply(KFOLD_df[, 2:6], 1, min)
kfold_mean_vec = apply(KFOLD_df[, 2:6], 1, mean)
{plot(x=KFOLD_df$Degree, 
     ...
     pch = 16,
     lwd=3,
     ylim=c(min(KFOLD_df[2:6]),max(KFOLD_df[2:6])))
lines(x=KFOLD_df$Degree,
     y=kfold_max_vec,
     col="red",
     lty=3,
     lwd=3)
...
# plot LOOCV
# calculate max and min limits of data
loocv_max_vec = apply(LOOCV_df[, 2:201], 1, max)
loocv_min_vec = apply(LOOCV_df[, 2:201], 1, min)
loocv_mean_vec = apply(LOOCV_df[, 2:201], 1, mean)
{plot(x=LOOCV_df$Degree,
     y=loocv_mean_vec,
     ylab="Mean Squared Error",
     main="LOOCV",
     xlab="Degree of Polynomial",
     ...
     pch = 16,
     lwd=3,
     ylim=c(min(LOOCV_df[,2:201]),max(LOOCV_df[,2:201])))
...
# plot LOOCV
# calculate max and min limits of data
loocv_max_vec = apply(LOOCV_df[,2:401], 1, max)
loocv_min_vec = apply(LOOCV_df[,2:401], 1, min)
loocv_mean_vec = apply(LOOCV_df[,2:401], 1, mean)
{plot(x=LOOCV_df$Degree,
     y=loocv_mean_vec,
     ylab="Mean Squared Error",
     main="LOOCV",
     ...
     pch = 16,
     lwd=3,
     ylim=c(min(LOOCV_df[2:401]),max(LOOCV_df[2:401])))
lines(x=LOOCV_df$Degree,
     y=loocv_max_vec,
     col="blue",
     lty=3,
