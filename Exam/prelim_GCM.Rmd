...
MCR <- function(pred, target){
  data_length = length(target)
  correct_classification = (target==pred)
  mcr = (data_length-sum(correct_classification))/data_length
  return(mcr)
}
```
...
nr = n*2
i = seq(5)
# FOR LOOP HERE AND GENERATE OUTPUT FOR EACH I
...
for(p_star in p_star_vec){
  feat_index_vec = c(top_features(pval_lst=t_test_results[[2]], p_val=0.05, p_min=p_star))
  assign(paste("feat_idx_vec_", p_star, sep = ""), feat_index_vec)
}

# create data set for each top selection
X_pstar_5 = X[,feat_idx_vec_5]
X_pstar_10 = X[,feat_idx_vec_10]
X_pstar_20 = X[,feat_idx_vec_20]
X_pstar_40 = X[,feat_idx_vec_40]

# Apply 10-fold CV to each data set
ids = myCVids(n=nr, K=10, seed=0)
k = 1
...
# END FOR LOOP HERE AND AVERAGRE MCR

# RECORD DATA ANAD SAVE OUTSIDE OF FOR LOOP
