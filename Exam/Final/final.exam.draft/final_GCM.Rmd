...
```{R}
# Bonus: Suppose another 50 predictors are added (uncorrelated with the response) 
# so that we now have
# 100 predictors. What do you expect to happen to the predictive performance of 
# the methods? Explore by
# simulating extra columns, generated as iid Uniform(-1,1).
...
# Problem 6
```{R}
 sqrt(mean((y_cv_valid_df - lr_pred)^2))
```
