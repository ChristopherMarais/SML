...
```{R}
# functions
MCR <- function(target, predicted){
  # Mis-classification ratio calculation
  if(length(target)!=length(predicted)){
    print("ERROR: predictions and targets not of the same shape!")
  }else{
    mcr = sum(predicted != target)/length(target)
    return(mcr)
  }
}
```
