...
k=2
data_df = data.frame(prob2.list[1])

# gmm portion
kmeans_res_lst = my_kmeans(data=df, k=k, seed=0)
...
cov_vec = c()
for(k_i in 1:k){
  df = data_df[class_vec==k_i,]
  cov_num = cov( df[2], df[1])[1]
  cov_vec = c(cov_vec, cov_num)
}
data_df[1,2]

log(dnorm(-3, mean=means_mat[1,1], sd=sqrt(sqrt(cov_vec[1]**2))))
mvrnorm(3, 0, 1)
# look at link in telegram
