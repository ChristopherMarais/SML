...
for(k_i in seq(k)){
  ...
  for(d in 1:d_max){
    ...
    for(k_j in seq(k)){
      ...
      lm.fit.j = lm(y ~ poly(x, d), data = data.train.j)
    ...
    lm.fit.i = lm(y ~ poly(x, d), data = data.train.i)
  ...
  best_d = (1:d_max)[which(rmse_d_vec == min(rmse_d_vec))]
...
results_df$best_d = 0
results_df$rmse_i = rmse_i_vec
for(i_k in 1:k){
  results_df$best_d[results_df$ki==i_k] = best_d_vec[i_k]
  # results_df$rmse_i[results_df$ki==i_k] = rmse_i_vec[i_k]
}

# collect data for curve plots from inner cv loop results
ki_d_rmse_lst = list()
for(k_i in 1:k){
  ki_d_rmse_vec = c()
  for(d in 1:d_max){
...
plot_df = data.frame((1:d_max), ki_d_rmse_lst)
colnames(plot_df) = c("d", as.character(1:k))
...
{plot(x=plot_df$d, 
     ...
     lwd=2, 
     lty=2,
     ylim=c(min(plot_df[2:5]),max(plot_df[2:5])), 
     pch=19,
     ylab="RMSE",
     xlab="Polynomial degree of  (d)")
