...
#####################################################
# get full pred_Y from folds
# get MCR at different thresholds for Y vs Y_pred

#####################################################       
    # ROC curve threshold loop
    # mcr_t_vec = c()
    # for(j in seq(1:length(threshold_vec))){
    #   t = threshold_vec[j]
    #   mcr_t = MCR(target=data_df[valid.k,]$Y, predicted=pred, threshold=t)
    #   mcr_t_vec = c(mcr_t_vec, mcr_t)
    #   ROC_df[valid.k,] = mcr_t_vec
    # }
    # mcr_t_df = data.frame(mcr_t_vec)
    # rownames(mcr_t_df) = threshold_vec
    # ROC_df[valid.k,j] = mcr_t_df
#####################################################    
  }
