...
data_df = data.frame(prob2.list[3])

# gmm portion
kmeans_res_lst = my_kmeans(data=data_df, k=k, seed=0)
means_mat = kmeans_res_lst[[1]]
class_vec = kmeans_res_lst[[2]]

# total covariance or mean covariance of clusters for LDA version?
cov_mat = cov(data_df) # likely ov for each cluster or mean of covs

# covariance of each cluster
# cov_lst = list()
# for(k_i in 1:k){
#   df = data.frame(data_df[class_vec==k_i,])
#   cov_mat = list(cov(df))
#   cov_lst = append(cov_lst, cov_mat)
# }

sample_i = 199
c1 = (dmvnorm(X=data_df[sample_i,], 
        mu=means_mat[1,],
        sigma=cov_mat))

c2 = (dmvnorm(X=data_df[sample_i,], 
        mu=means_mat[2,],
        sigma=cov_mat))

c_all = c1+c2
print(paste(c1, c2, c1/c_all, c2/c_all, class_vec[sample_i]))



#use sqrt of var in table for each
# log dnorm
# log(dnorm(-3, mean=means_mat[1,1], sd=sqrt(cov_lst[1][1])))
```
...
for( i in seq(1:length(prob2.list))){
  kmeans_res_lst = my_kmeans(data=data_df, k=k, seed=0)
  plot(data.frame(prob2.list[i]), col=kmeans_res_lst[2])  
}
