...
ff <- function(a,b) {nbSubsets(a,b)}

# function to convert binary representation to decimal representation 
bin2dec <- function(binM) { # vectorized implementation;
  if (!is.matrix(binM)) {binM = matrix(binM, nrow=1)} # if binM is a vector, treat it as a matrix;
